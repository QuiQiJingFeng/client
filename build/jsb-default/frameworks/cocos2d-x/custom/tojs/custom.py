...
def main():
    ...
    jsb_root = os.path.abspath(os.path.join(project_root, 'custom/binding/'))
    ...
    try:
        tojs_root = '%s/custom/tojs' % project_root
        output_dir = '%s/custom/binding' % project_root

        cmd_args = {
                        'custom.ini' : ('custom', 'jsb_custom_auto'),
                    }
        target = 'spidermonkey'
        ...
        if len(custom_cmd_args) > 0:
            output_dir = '%s/frameworks/custom/' % project_root
